...
# inisialisasi prolog
prolog = Prolog()
prolog.consult("pakar_pernapasan_gui.pl")
...
def pertanyaan_selanjutnya(ganti_penyakit = False):
    ...
    # apabila semua gejala dari penyakit habis, berarti terdeteksi penyakit tersebut
...
def hasil_diagnosa(penyakit = ""):
    if penyakit:
        penjelasan = ""
        if  penyakit == "Asma":
            penjelasan = "Asma adalah kondisi jangka panjang yang menyebabkan saluran napas bengkak dan sensitif. Harap konsultasikan dengan dokter untuk penanganan lebih lanjut."
        elif penyakit == "Bronkitis":
            penjelasan = "Bronkitis adalah peradangan saluran pernapasan (bronkus). Istirahat, minum banyak cairan, dan konsultasi dengan dokter diperlukan."
        elif penyakit == "Pneumonia":
            penjelasan = "Pneumonia adalah infeksi yang menyebabkan peradangan di kantong-kantong udara paru-paru. Segera cari pertolongan medis."
        elif penyakit == "PPOK":
            penjelasan = "Penyakit Paru Obstruktif Kronis (PPOK) adalah kelompok penyakit paru yang menyebabkan kesulitan bernapas. Konsultasi dengan spesialis paru diperlukan."
            
        messagebox.showinfo("Hasil Diagnosa", f"Anda terdeteksi menderita {penyakit}.\n\n{penjelasan}")
    else:
        messagebox.showinfo("Hasil Diagnosa", "Tidak terdeteksi penyakit pernapasan berdasarkan gejala yang Anda berikan.")
...
root = tk.Tk()
root.title("Sistem Pakar Diagnosis Penyakit Pernapasan")
...
ttk.Label(mainframe, text="Aplikasi Diagnosa Penyakit Pernapasan", font=("Arial", 16)).grid(column=0, row=0, columnspan=3)
